...
describe('general term-vectoriness:', function () {
  ...
  it('should make nice term vectors for ascii text when vector length is 1', function () {
    var vec = tv.getVector('This is cool')
    vec.should.eql([
      [ [ 'cool' ], 1 ]
    ])
  })
  ...
  it('should remove stopwords', function () {
    tv.getVector('one of the best vectors that should exist').should.eql([
      [ [ 'best' ], 1 ],
      [ [ 'exist' ], 1 ],
      [ [ 'one' ], 1 ],
      [ [ 'vectors' ], 1 ]
  ...
  it('should return norwegian stopwords', function () {
    var sw = tv.getStopwords('no')
    sw.should.be.instanceof(Array).and.have.lengthOf(129)
    sw[0].should.be.exactly('og')
    sw[1].should.be.exactly('i')
    sw[2].should.be.exactly('jeg')
    sw[3].should.be.exactly('det')
  })
  ...
  it('should be possible to customise seperators (exclude spaces)', function () {
    ...
    var vec = tv.getVector(text, options)
    vec.should.eql([
      [ [ 'gerät kjærsti grünnerløkka kjærsti' ], 1 ],
